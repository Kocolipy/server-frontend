...
function geoMap(data) {
    ...
    for (i=0; i<airports.length;i++) {
        var airportElement = airports[i];

        airportElement['svgPath'] = targetSVG;
        dataPlot['images'].push(airportElement);
    }

    dataPlot['lines'] = [];
    for (i=0;i<aircraft_routes.length;i++) {
        var routeElement = aircraft_routes[i];

        var routeId = routeElement['id'];
        var aircraftIcon ={
            "svgPath": planeSVG,
            "positionOnLine": 0,
            "color": "#585869",
