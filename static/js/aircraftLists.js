...
/* adds event listener on the dropdown of the Insights panel
 * it handles dynamically added items*/
function addListenersToDropdownItems() {
    ...
    dropMenu.addEventListener("click", function (e) {
        if (e.target && e.target.matches("li.highlight-on-hover")) {
            ...
            clearCache();
            var clickedAircraftID = "?engine=" + e.target.innerHTML.split(' ')[1];
            getEngineFromBackEnd(displayInfoToUser, clickedAircraftID, "insightsUserMessage");
            document.getElementById('dropDownButton').innerHTML = "Selected: " +  e.target.innerHTML
                + "<span class=\"glyphicon glyphicon-menu-down\" style=\"margin-left: 20px \"></span>";
        }
    });
}
